...
We heavily used plyj to convert Java code into an AST. Plyj is a version of Ply that parses Java code. Ply is a lexical/parsing (Lex/Yacc) tool written in Python.  Some of the features in ply include standard lexical analysis and parsing features: error checking, AST tree generation, and language-specific capability.
